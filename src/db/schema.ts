...
export const profileVerificationStatusEnum = pgEnum(
...
export const RulesConfigSchema = z.object({
	timeFormat: z
		.enum(["HH:MM:SS", "HH:MM:SS.sss", "MM:SS.sss"])
		.default("HH:MM:SS"),
...
export const courses = pgTable(
	"courses",
	{
		...
		copiedFromCourseId: uuid("copied_from_course_id").references(
 ...
 */
export const timingPoints = pgTable(
	"timing_points",
	{
		id: uuid("id").defaultRandom().primaryKey(),
		organizationId: uuid("organization_id")
 ...
 */
export const courseSplits = pgTable(
	"course_splits",
	{
		id: uuid("id").defaultRandom().primaryKey(),
		courseId: uuid("course_id")
			...
			.notNull(),
		timingPointId: uuid("timing_point_id")
			.references(() => timingPoints.id, { onDelete: "restrict" })
			.notNull(),
		name: text("name").notNull(), // e.g. "Twin Lakes Inbound"
		sequenceOrder: integer("sequence_order").notNull(), // 1, 2, 3...
		distanceFromStartMeters: doublePrecision(
		...
		isSecretCheckpoint: boolean("is_secret_checkpoint")
			.default(false)
			.notNull(),
	...
	(t) => [
		...
		index("course_splits_course_dist_idx").on(
			t.courseId,
			t.distanceFromStartMeters,
		),
	],
);
...
export const entrants = pgTable(
	"entrants",
	{
		...
		emergencyContactPersonId: uuid("emergency_contact_person_id").references(
			() => people.id,
			{ onDelete: "set null" },
		),
		emergencyContactRelationship: text("emergency_contact_relationship"),
		emergencyContactIsOnSite: boolean("emergency_contact_is_on_site")
			.default(false)
			.notNull(),
		...
		// DNF / Drop Details
		droppedSplitId: uuid("dropped_split_id").references(() => courseSplits.id, {
			onDelete: "set null",
		}),
	...
	(t) => [
		uniqueIndex("entrants_event_competitor_idx").on(
			t.eventId,
			t.competitorNumber,
		),
		index("entrants_transponder_idx").on(t.rfidCode),
		index("entrants_status_idx").on(t.eventId, t.status),
		index("entrants_emergency_contact_person_idx").on(
			t.emergencyContactPersonId,
		),
	],
);
 ...
 */
export const rawTimingEvents = pgTable(
	"raw_timing_events",
	{
		id: uuid("id").defaultRandom().primaryKey(),
		eventGroupId: uuid("event_group_id")
		...
		recordedTimeUtc: timestamp("recorded_time_utc", {
			withTimezone: true,
			precision: 3,
			mode: "date",
		}).notNull(),
 ...
 */
export const scoredSplitTimes = pgTable(
	"scored_split_times",
	{
		id: uuid("id").defaultRandom().primaryKey(),
		entrantId: uuid("entrant_id")
			...
			.notNull(),
		courseSplitId: uuid("course_split_id")
			.references(() => courseSplits.id, { onDelete: "cascade" })
			.notNull(),
		lap: integer("lap").default(1).notNull(),
		subSplitKind: subSplitKindEnum("sub_split_kind").notNull(),

		// High Precision Metrics (Millisecond Precision)
		absoluteTimeUtc: timestamp("absolute_time_utc", {
			withTimezone: true,
			precision: 3,
			mode: "date",
		}).notNull(),
		...
		governingRawEventId: uuid("governing_raw_event_id").references(
			() => rawTimingEvents.id,
			{ onDelete: "set null" },
		),
	...
	(t) => [
		uniqueIndex("scored_split_unique_idx").on(
			t.entrantId,
			t.courseSplitId,
			t.lap,
			t.subSplitKind,
 ...
 */
export const timeAllowances = pgTable(
	"time_allowances",
	{
		id: uuid("id").defaultRandom().primaryKey(),
		entrantId: uuid("entrant_id")
	...
	(t) => [index("time_allowances_entrant_idx").on(t.entrantId)],
);

/**
 * Forensic Audit Trail: Captures every status change or timestamp modification.
 */
export const splitTimeAudits = pgTable(
	"split_time_audits",
	{
		id: uuid("id").defaultRandom().primaryKey(),
		splitTimeId: uuid("split_time_id")
			.references(() => scoredSplitTimes.id, { onDelete: "cascade" })
			.notNull(),
	...
	(t) => [index("split_time_audits_split_idx").on(t.splitTimeId)],
);
...
export const personExternalProfiles = pgTable(
	"person_external_profiles",
	{
		...
		verificationStatus: profileVerificationStatusEnum("verification_status")
			.default("unverified")
			.notNull(),
	...
	(t) => [
		uniqueIndex("person_external_profiles_platform_identifier_idx").on(
			t.platform,
 ...
 */
export const personRaceHistory = pgTable(
	"person_race_history",
	{
		id: uuid("id").defaultRandom().primaryKey(),
		personId: uuid("person_id")
			.references(() => people.id, { onDelete: "cascade" })
			.notNull(),
		externalProfileId: uuid("external_profile_id").references(
			() => personExternalProfiles.id,
			{ onDelete: "set null" },
		),
		platform: externalPlatformEnum("platform").notNull(),
 ...
 */
export const personHistoricalSummaries = pgTable(
	"person_historical_summaries",
	{
		id: uuid("id").defaultRandom().primaryKey(),
		personId: uuid("person_id")
			.references(() => people.id, { onDelete: "cascade" })
		...
		maxDistanceMeters: doublePrecision("max_distance_meters")
			.default(0)
			.notNull(),
		maxElevationGainMeters: doublePrecision("max_elevation_gain_meters")
			.default(0)
			.notNull(),
		avgPaceSecondsPerKm: doublePrecision("avg_pace_seconds_per_km"),
		experienceTier: experienceTierEnum("experience_tier")
			.default("novice")
			.notNull(),
 ...
 */
export const entrantHistoricalProjections = pgTable(
	"entrant_historical_projections",
	{
		...
		projectedArrivalTimes: jsonb("projected_arrival_times").$type<
			Record<string, { secondsFromStart: number; projectedTimeUtc: string }>
		>(),
		...
		expectedCutoffRisk: boolean("expected_cutoff_risk")
			.default(false)
			.notNull(),
		volunteersSarSafetyBadge: text("volunteers_sar_safety_badge"),
		projectionMethod: text("projection_method")
			.default("historical_pacing")
			.notNull(),
		computedAt: timestamp("computed_at", { withTimezone: true, mode: "date" })
			.defaultNow()
			.notNull(),
	...
	(t) => [
		...
		index("entrant_historical_projections_cutoff_risk_idx").on(
			t.expectedCutoffRisk,
		),
	],
);

// ============================================================================
// DEMO / EXISTING TABLE PRESERVATION
// ============================================================================
...
export const timingPointsRelations = relations(
	timingPoints,
	({ one, many }) => ({
		organization: one(organizations, {
...
export const courseSplitsRelations = relations(
	courseSplits,
	({ one, many }) => ({
		course: one(courses, {
...
export const peopleRelations = relations(people, ({ many, one }) => ({
	...
	emergencyContactForEntrants: many(entrants, {
...
export const personExternalProfilesRelations = relations(
...
export const personRaceHistoryRelations = relations(
	personRaceHistory,
	({ one }) => ({
		person: one(people, {
...
export const personHistoricalSummariesRelations = relations(
...
export const entrantHistoricalProjectionsRelations = relations(
...
export const rawTimingEventsRelations = relations(
	rawTimingEvents,
	({ one, many }) => ({
		eventGroup: one(eventGroups, {
...
export const scoredSplitTimesRelations = relations(
	scoredSplitTimes,
	({ one, many }) => ({
		entrant: one(entrants, {
...
export const splitTimeAuditsRelations = relations(
	splitTimeAudits,
	({ one }) => ({
...
export type PersonExternalProfile = InferSelectModel<
	typeof personExternalProfiles
>;
export type NewPersonExternalProfile = InferInsertModel<
	typeof personExternalProfiles
>;
...
export type PersonHistoricalSummary = InferSelectModel<
	typeof personHistoricalSummaries
>;
export type NewPersonHistoricalSummary = InferInsertModel<
	typeof personHistoricalSummaries
>;

export type EntrantHistoricalProjection = InferSelectModel<
	typeof entrantHistoricalProjections
>;
export type NewEntrantHistoricalProjection = InferInsertModel<
	typeof entrantHistoricalProjections
>;
...
export type HelloWorld = InferSelectModel<typeof helloWorld>;
export type NewHelloWorld = InferInsertModel<typeof helloWorld>;
